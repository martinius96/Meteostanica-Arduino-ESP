...
include 'connect.php';
 	...
 	$teplota1 = mysqli_query($con,"SELECT teplota2 FROM vencurik ORDER BY id DESC LIMIT 1") or die(mysqli_error($con));
		while($line = mysqli_fetch_assoc($teplota1)){
		
			$prvateplota = $line['teplota2'];

		}
		$teplota1druha = mysqli_query($con,"SELECT teplota2 FROM vencurik WHERE id=(SELECT MAX(id)-1 FROM vencurik)") or die(mysqli_error($con));
		while($line = mysqli_fetch_assoc($teplota1druha)){
		
			$druhateplota =$line['teplota2'];
