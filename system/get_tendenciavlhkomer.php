...
include 'connect.php';
 	...
 	$teplota1 = mysqli_query($con,"SELECT vlhkost FROM vencurik ORDER BY id DESC LIMIT 1") or die(mysqli_error($con));
		while($line = mysqli_fetch_assoc($teplota1)){
		
			$prvateplota = $line['vlhkost'];

		}
		$teplota1druha = mysqli_query($con,"SELECT vlhkost FROM vencurik WHERE id=(SELECT MAX(id)-1 FROM vencurik)") or die(mysqli_error($con));
		while($line = mysqli_fetch_assoc($teplota1druha)){
		
			$druhateplota =$line['vlhkost'];
...
if($prvateplota==$druhateplota) { 
  $tie = '<img src="https://image.flaticon.com/icons/svg/179/179385.svg" alt="Tie" title="Ustálená - Rozdiel 0%" height="32" width="32">';  
  echo $tie; }
?>
